...
from services.summary_service.app.service import get_summary_service
from shared.config.settings import get_summary_service_settings
from shared.logging.json_logger import build_json_logger
from shared.schemas.gateway import ErrorResponse, HealthResponse
from shared.schemas.summary import SummaryRequest, SummaryResponse
from shared.utils.request_id import REQUEST_ID_HEADER, resolve_request_id


settings = get_summary_service_settings()
logger = build_json_logger(settings.service_name, settings.log_level)

app = FastAPI(
    title="SupportCopilot Summary Service",
    description="Summary generation microservice for the SupportCopilot architecture.",
    version="0.1.0",
)


def request_to_sample(payload: SummaryRequest) -> dict:
    return {
...
@app.post("/api/v1/summary", response_model=SummaryResponse, tags=["summary"])
def generate_summary_endpoint(payload: SummaryRequest, request: Request, response: Response) -> SummaryResponse:
